...
#TABLE 1
#loading data on birds with literature
divingdf<-read.csv("~/Uni/PROJECT/SoaneDepthFinal.csv")
#loading data on birds without literature
divingdfb<-read.csv("~/Uni/PROJECT/SpeciesNoLiterature.csv")
#loading dplyr for data handling
library(dplyr)
...
#rnaturaldata has global maps
library(rnaturalearth)
library(rnaturalearthdata)
#packages for plot aesthetics
library(ggplot2)
library(cowplot)
...
#import data
divingdf<-read.csv("~/Uni/PROJECT/SoaneDepthFinal.csv")
#load ggplot2 for my graph aesthetics
library(ggplot2)
#adding 1 to every diving depth because 0 values cannot be logged
...
#loading the new mean temperature tif
...
#installing package to deal with ncdf files
...
#reloading the dataset each time because I have to get rid of NAs
divingdf<-read.csv('~/Uni/PROJECT/SoaneDepthFinal.csv')
...
#find minimum, maximum and mean values
...
#how skewed is the mean?
(2.15/61)*100  #3.52 very low compared to maximum value
...
#how skewed is the mean?
(0.17/5.17)*100 #3.29 not quite as skewed
...
#below 5 indicating low correlation
...
#omitting NAs and subsetting the dataset
...
#weight.kg is body mass in report
...
#load dataset
divingdf<-read.csv("~/Uni/PROJECT/SoaneDepthFinal.csv")
#loading lme4 for GLMs
library(lme4)
#most saturated model with chlorophyll concentration 
...
#most saturated model with turbidity index
...
#simple one coefficient model
b4<- glm(Depthzero ~ Surface.Temperature, data = divingdf, family = binomial(link = logit))
#looking at summaries of all four models
summary(b)
summary(b2)
summary(b3)
summary(b4)
...
#TABLE 2 
#statistics on binomial GLM fit
#using the first model with chlorophyll concentration
#pseudo r-squared
...
#TABLE 3 
#coefficient summary of binomial GLM
summary(b)
#finding confidence intervals for the coefficient table
confint(b)
...
#adding the log variables onto the dataset for LLMs
...
#load package
library(MuMIn)
#find R-square values for LLM
r.squaredGLMM(g)



#FIGURE S3
#load package that makes coefficient plot
library(coefplot)
...
#FIGURE S4
#installing package that calculates a residual plot for LLMs
